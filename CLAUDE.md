...
- **`scripts/`** — the actual Lutris installer YAMLs. These are the current,
  maintained product (native-Linux-manager version). Inline comments in the
  YAML should explain non-obvious choices for the next maintainer, not
  narrate what each task does.
- **`archive/`** — the previous, Wine-based generation of this same
  workflow (its own `README.md` and `scripts/`), kept as a fallback in case
  the native-build approach in the current `scripts/` doesn't pan out. It's
  frozen: don't fix bugs or add features there, and don't let it drift out
  of sync with the current design's rationale — if something changes in
  `docs/DESIGN-NOTES.md#relationship-to-archive`, keep that section
  accurate instead of touching `archive/` itself.
